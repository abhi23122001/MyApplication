...
fun FinancialReportScreen(onBack: () -> Unit) {
    ...
    LaunchedEffect(Unit) {
        db.collection("projects").get().addOnSuccessListener { projects = it.documents.map { d -> d.data.orEmpty() + mapOf("id" to d.id) } }.addOnFailureListener { error = it.message }
        db.collection("payments").get().addOnSuccessListener { payments = it.documents.map { d -> d.data.orEmpty() } }.addOnFailureListener { error = it.message }
        db.collection("expenses").get().addOnSuccessListener { expenses = it.documents.map { d -> d.data.orEmpty() } }.addOnFailureListener { error = it.message }
    }
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("Financial Reports") }, navigationIcon = { IconButton(onClick = onBack) { Icon(Icons.AutoMirrored.Filled.ArrowBack, "Back") } }) }) { p ->
        LazyColumn(Modifier.fillMaxSize().padding(p).padding(horizontal = 16.dp), contentPadding = PaddingValues(bottom = 24.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            items(projects, key = { text(it, "id", "name") }) { project ->
...
fun WorkProgressReportScreen(onBack: () -> Unit) {
    val db = remember { FirebaseFirestore.getInstance() }; var projects by remember { mutableStateOf<List<Map<String, Any>>>(emptyList()) }; var tasks by remember { mutableStateOf<List<Map<String, Any>>>(emptyList()) }; var error by remember { mutableStateOf<String?>(null) }
    LaunchedEffect(Unit) { db.collection("projects").get().addOnSuccessListener { projects = it.documents.map { d -> d.data.orEmpty() + mapOf("id" to d.id) } }.addOnFailureListener { error = it.message }; db.collection("tasks").get().addOnSuccessListener { tasks = it.documents.map { d -> d.data.orEmpty() } }.addOnFailureListener { error = it.message } }
    val completedProjects = projects.count { project -> val id=text(project,"id"); val name=text(project,"name","projectName"); val list=tasks.filter{text(it,"projectId","project_id")==id||text(it,"projectName","project")==name}; list.isNotEmpty() && list.all{text(it,"status").equals("Completed",true)||text(it,"status").equals("Complete",true)} }
    Scaffold(topBar = { TopAppBar(title = { Text("Work Progress") }, navigationIcon = { IconButton(onClick = onBack) { Icon(Icons.AutoMirrored.Filled.ArrowBack,"Back") } }) }) { p ->
        LazyColumn(Modifier.fillMaxSize().padding(p).padding(horizontal=16.dp), contentPadding=PaddingValues(bottom=24.dp), verticalArrangement=Arrangement.spacedBy(12.dp)) {
            ...
            items(projects,key={text(it,"id","name")}) { project -> val id=text(project,"id");val name=text(project,"name","projectName").ifBlank{"Project"};val list=tasks.filter{text(it,"projectId","project_id")==id||text(it,"projectName","project")==name};val done=list.count{text(it,"status").equals("Completed",true)||text(it,"status").equals("Complete",true)};val progress=if(list.isEmpty())0f else done.toFloat()/list.size;Card(Modifier.fillMaxWidth(),shape=RoundedCornerShape(18.dp)){Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(7.dp)){Text(name,style=MaterialTheme.typography.titleMedium,fontWeight=FontWeight.Bold);LinearProgressIndicator(progress={progress},Modifier.fillMaxWidth());Text("${(progress*100).toInt()}% complete  •  ${list.size-done} pending");if(list.isNotEmpty()&&done==list.size)Text("WORK COMPLETED",fontWeight=FontWeight.Bold,color=ShahGreen)}} }
            item { error?.let { Text(it,color=MaterialTheme.colorScheme.error) } }
        }
    }
}
