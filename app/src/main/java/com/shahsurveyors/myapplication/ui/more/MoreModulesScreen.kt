...
fun MoreModulesScreen(onNavigate: (String) -> Unit) {
    Scaffold(topBar = {
        TopAppBar(title = { Column { Text("More Modules", fontWeight = FontWeight.Bold, color = ShahWhite); Text("All tools & business modules", fontSize = 10.sp, color = ShahWhite.copy(alpha = .72f)) } }, colors = TopAppBarDefaults.topAppBarColors(containerColor = ShahDarkGreen))
    }) { paddingValues ->
            ...
            item {
                Surface(Modifier.fillMaxWidth(), RoundedCornerShape(18.dp), color = ShahDarkGreen) {
                    Row(Modifier.padding(18.dp), verticalAlignment = Alignment.CenterVertically) {
                        Surface(shape = RoundedCornerShape(14.dp), color = ShahWhite.copy(alpha = .12f)) { Icon(Icons.Default.Apps, null, tint = ShahWhite, modifier = Modifier.padding(11.dp).size(28.dp)) }
                        Spacer(Modifier.width(13.dp)); Column { Text("Everything in one place", color = ShahWhite, fontSize = 17.sp, fontWeight = FontWeight.Bold); Spacer(Modifier.height(3.dp)); Text("Choose a module to continue", color = ShahWhite.copy(alpha = .72f), fontSize = 11.sp) }
                    }
                }
            }
...
val peopleModules = listOf(
    ...
    ModuleData("Advance Salary", "Request salary advance for approval", Icons.Default.Payments, "advance_salary"),
