...
class EquipmentViewModel(private val repository: EquipmentRepository) : ViewModel() {
 ...
 fun returnEquipment(id:String,location:String,remarks:String){if(location.isBlank()){errorMessage="Please enter return location";return};viewModelScope.launch{isLoading=true;errorMessage=null;try{val admin=FirebaseAuth.getInstance().currentUser;repository.returnEquipment(id,location,remarks,admin?.uid?:"ADMIN",admin?.displayName?:"Admin");fetchEquipment()}catch(e:Exception){errorMessage=e.localizedMessage?:"Unable to return equipment"}finally{isLoading=false}}}
