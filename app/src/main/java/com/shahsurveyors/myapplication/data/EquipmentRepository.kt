...
class EquipmentRepository(private val firestore: FirebaseFirestore = FirebaseFirestore.getInstance()) {
    ...
    suspend fun saveHandover(id:String,employee:UserProfile,project:ProjectModel,location:String,accessories:List<String>,handoverByUid:String,handoverByName:String) {
        ...
        historyCollection.document().set(mapOf("equipmentId" to id,"equipmentName" to (getAllEquipment().firstOrNull{it.id==id}?.name ?: ""),"action" to "HANDOVER","employeeUid" to employee.uid,"employeeName" to employee.name,"projectId" to project.id,"projectName" to project.name,"location" to location.trim(),"accessories" to cleanAccessories,"performedByUid" to handoverByUid,"performedByName" to handoverByName,"timestamp" to now)).await()
    }
    ...
    suspend fun returnEquipment(id:String,location:String,remarks:String) {
        ...
        historyCollection.document().set(mapOf("equipmentId" to id,"equipmentName" to (current?.name ?: ""),"action" to "RETURN","employeeUid" to current?.assignedToUid,"employeeName" to current?.assignedToName,"projectId" to current?.assignedProjectId,"projectName" to current?.assignedProjectName,"location" to location.trim(),"accessories" to (current?.handoverAccessories ?: emptyList<String>()),"remarks" to remarks.trim(),"performedByUid" to (admin?.uid ?: "ADMIN"),"performedByName" to (admin?.displayName ?: "Admin"),"timestamp" to now)).await()
